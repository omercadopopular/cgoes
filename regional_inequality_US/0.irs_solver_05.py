...
FILES = irs_walker('/Users/cbezerra/Documents/IRS/2005/')
NBRACKETS = 6
SKIP_ROWS = 18
FOOTNOTES = 9
...
for file in FILES:
    print(file)
    if file.split('.')[0][-2:].upper() == 'AL':
        TEMP = irs_solver(file, NBRACKETS, SKIP_ROWS+1, FOOTNOTES)
    elif file.split('.')[0][-2:].upper() == 'WV':
        TEMP = irs_solver(file, NBRACKETS, SKIP_ROWS, 2)
    else: 
        TEMP = irs_solver(file, NBRACKETS, SKIP_ROWS, FOOTNOTES)
    DF = DF.append(TEMP)
    
DF.to_csv('/Users/cbezerra/Documents/IRS/2005.csv', sep=',')