...
var y c k i n w r gc taun gi gk;  
...
parameters tauk deltag theta varphi;  
parameters ky_ss iy_ss cy_ss gcy_ss gc_ss n_ss y_ss r_ss varphi_ss varphiy_ss w_ss giy_ss gky_ss gi_ss gk_ss taun_ss;
...
varphi = -0.748901727484595;                % lump-sum tax
tauk = .05;                % capital-income tax rate
...
varphiy_ss = -0.135533443948979;
taun_ss = (-tauk*(r_ss-delta)*ky_ss + giy_ss + gcy_ss + varphiy_ss)/(1-alpha); // labor-tax transfers

cy_ss = 1 - gcy_ss - giy_ss - iy_ss; // steady state private consumption to output ratio

n_ss = ((1/psi) * cy_ss^(-sigma) * (1-taun_ss) * (1-alpha))^((1-alpha-theta)/((1-alpha-theta)*(1+phi)-(1-alpha)*(1-sigma))) * (z * ky_ss^alpha * gky_ss^theta )^((1-sigma)/((1-alpha-theta)*(1+phi) - (1-alpha)*(1-sigma))); // SS hours
...
A = [oo_.exo_simul, gc, y, c, i, n, k, r] ;
