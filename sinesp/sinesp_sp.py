...
def plot_figure(frame, file_name):
    ...
    fig.suptitle("Estado de São Paulo: Ocorrências no Sinesp (Ministério da Justiça). Elaborado por @goescarlos", fontsize=22, fontweight='bold')
    ...
    fig.savefig(os.path.join(path, f'{file_name}_sp.pdf'))
    fig.savefig(os.path.join(path, f'{file_name}_sp.png'))
...
for filter in filters:
    counter += 1
    frame_filter = frame[frame.index.isin(filter)].fillna(0)
    plot_figure(frame_filter, f'sinesp{counter}_sp')
