...
set matsize 11000
*set max_memory 5g
log using `year'.log, replace  					// chooses logfile
...
if (pppadjustment == 1) {
	...
	// 1. Estimate cost of living adjustments by state and rural area
		...
		** 1.4. Create PPP index
			...
			gen pppindex =  ( 0.5554 * rentindex ) + 1
...
///////////////////////////////////////////////////////////////////////////////
	...
	preserve
		...
		export delimited using "percentiles", replace
	restore
	...
	preserve
		...
		export delimited using "incomepercentiles", replace
	restore

	preserve
		collapse (mean) income, by(pctilestate)
		export delimited using "incomepercentilestotal", replace
	restore	
...
preserve
	...
	twoway scatter pctile pctilestate if UF == 11, msize(small)  || ///
		...
		graph export north.pdf, as(pdf) replace
	...
	twoway scatter pctile pctilestate if UF == 21, msize(small)  || ///
		...
		graph export northeast.pdf, as(pdf) replace
	...
	twoway scatter pctile pctilestate if UF == 31, msize(small)  || ///
		...
		graph export southeast.pdf, as(pdf) replace
	...
	twoway scatter pctile pctilestate if UF == 41, msize(small)  || ///
		...
		graph export south.pdf, as(pdf) replace
	...
	twoway scatter pctile pctilestate if UF == 50, msize(small)  || ///
		...
		graph export midwest.pdf, as(pdf) replace
	...
	twoway scatter pctile pctilestate if UF == 50, msize(small) mcolor(red)  || ///
		...
		graph export full.pdf, as(pdf) replace
...
preserve
	...
	// Stove
		...
		replace stove = 0 if  V0222 == 4 | V0221 == 3
			...
			export delimited using "consumption", replace
...
preserve
	...
	export delimited using incomedensity, replace	
restore
...
// Calculate inequality indices
	...
	// Store results
		...
		local list = "between_a1 within_a1 between_a2 within_a2 between_ge2 within_ge2 between_ge1 within_ge1 between_ge0 within_ge0 gini p90p10 p75p50 sd mean"
		local counter = 0
		
		// Overall

		qui ineqdeco income [aweight=`weight'] , by(UF)
		putexcel A1=rscalarnames using giniresults`year'.xlsx, replace sheet("overall")
		putexcel B1=rscalars using giniresults`year'.xlsx, modify sheet("overall")
		
		histogram lincome, ///
			ytitle("Density") ///
			xtitle("Log of HH income") ///
			title("BR: HH Income per Capita Distribution, `year'", margin(vsmall) position(11)) ///
			lwidth(thin) scheme(s2color) name(histogramoverall, replace)
			graph export histogramoverall.pdf, as(pdf) replace
		...
		// 5 percentile
			...
			preserve
				...
				keep if pctilestate == 0.05 *  `ntiles'

				qui ineqdeco income , by(UF)
			
				putexcel A1=rscalarnames using giniresults`year'.xlsx, modify sheet("bottom")
				putexcel B1=rscalars using giniresults`year'.xlsx, modify sheet("bottom")
									 
			histogram lincome, ///
				ytitle("Density") ///
				xtitle("Log of HH income") ///
				title("Bottom: HH Income per Capita Distribution, `year'", margin(vsmall) position(11)) ///
				lwidth(thin) scheme(s2color) name(histogramBottom, replace)
				graph export histogramBottom.pdf, as(pdf) replace
		...
		// 50 percentile
			...
			preserve
				...
				keep if pctilestate == 0.5 *  `ntiles'

				qui ineqdeco income [aweight=`weight'] , by(UF)
			
				putexcel A1=rscalarnames using giniresults`year'.xlsx, modify sheet("middle")
				putexcel B1=rscalars using giniresults`year'.xlsx, modify sheet("middle")							 
												 
			histogram lincome, ///
				ytitle("Density") ///
				xtitle("Log of HH income") ///
				title("Middle: HH Income per Capita Distribution, `year'", margin(vsmall) position(11)) ///
				lwidth(thin) scheme(s2color) name(histogramMiddle, replace)
				graph export histogramMiddle.pdf, as(pdf) replace
		...
		// 95 percentile
			...
			preserve
				...
				keep if pctilestate == 0.95 * `ntiles'

				qui ineqdeco income [aweight=`weight'] , by(UF)			
			
				putexcel A1=rscalarnames using giniresults`year'.xlsx, modify sheet("top")
				putexcel B1=rscalars using giniresults`year'.xlsx, modify sheet("top")							 
												 

			histogram lincome, ///
				ytitle("Density") ///
				xtitle("Log of HH income") ///
				title("Top: HH Income per Capita Distribution, `year'", margin(vsmall) position(11)) ///
				lwidth(thin) scheme(s2color) name(histogramtop, replace)
				graph export histogramtop.pdf, as(pdf) replace
