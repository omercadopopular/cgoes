...
function logacr!(out, rr, Am, am, Id, p, H, d, λk)
...
function welfare(p, H, β, regs)
    ...
    for d in 1:p.N
        logacr!(out, 1+(d-1)*H:d*H, Am, am, Id, p, H, d, λk)
    end
    df = DataFrame(out, [:flex, :dist, :distk, :distn, :etasum])
...
function main()
    ...
    #=
    p = Problem(T=H)
    paras = elas["theta_sigma_zeta_blp_h_eff"][1:3,1:3]
    ζs = [0.2, 0.4, paras[3,1]]
    θs = [(paras[1,1] for _ in 1:2)..., 5.0]
    paras = hcat(paras, fill(paras[2,1], 3, 3))
    paras[1,4:end] .= θs
    paras[3,4:end] .= ζs
...
=#
    ...
    for (i, suf) in enumerate(sufs)
        para = view(paras, :, i)
        println("\n theta = ", para[1], " sigma = ", para[2], " zeta = ", para[3])
        println()
        # p = Problem(T=H)
        # run!(p, elas, parajd, paraijd, paraisd, mtau, xtau, dreg, regs, secs, itau, H, β, tag="base");
        # p = Problem(T=H)
        # run!(p, elas, parajd, paraijd, paraisd, mtau, xtau, dreg, regs, secs, itau, H, β, noadj=true, tag="baselr")

        # Verify that the ACR result matches model result
        # dfw[(abs.(dfw.flex .+ dfw.dist .-dfw.wp).>1e-5).&(dfw.flex.!=0),:]
    ...
    #para = (2.5419, 1.1, 0.0756)
    
    #para = (6.5419, 2.1, 0.0756)
    ...
    run!(p, para, parajd, paraijd, paraisd, mtau, xtau, dreg, regs, secs, itau, H, β,
            fillηG=true, tag="Gtry3", saveresult=false)
    #=p1 = Problem(T=H, sc=MaxPresentValue(T=H, TΨ=H-20))
    run!(p1, para, parajd, paraijd, paraisd, mtau, xtau, dreg, regs, secs, itau, H, β,
            noadj=true, fillηG=true, tag="Glrtry3")=#
    p2 = Problem(T=H)
    run!(p2, para, parajd, paraijd, paraisd, mtau, xtau, dreg, regs, secs, itau, H, β,
            fillηG=true, tag="Gtry03", saveresult=false)
    p3 = Problem(T=H)
    run!(p3, para, parajd, paraijd, paraisd, mtau, xtau, dreg, regs, secs, itau, H, β,
        noadj=true, fillηG=true, tag="Glrtry03", saveresult=false)

    #=
    p = Problem(T=40)
    run!(p, para, parajd, paraijd, paraisd, mtau, xtau, dreg, regs, secs, itau, H, β,
        noadj=true, fillηG=true, tag="Glrtry0")=#
    return nothing
end

#@time main()
#=
     7     1.285724e-06     9.588471e-08
solveΨ! converged after 7 iterations
1141.777450 seconds (6.89 M allocations: 5.821 GiB, 0.04% gc time, 0.30% compilation time)
=#

#=
857.489927 seconds (7.47 M allocations: 5.249 GiB, 0.05% gc time, 0.23% compilation time)
=#

#=
ir = 1:20
r = 15
plot(100.0 .*[p.w[r,ir] p.sc.dΠ[r,ir] p.drealincome[r,ir]], labels=["Wage" "Profit" "Total"], title="China")
savefig("figtemp/dincomebytypeCHN1.pdf")
r = 77
plot(100.0 .*[p.w[r,ir] p.sc.dΠ[r,ir] p.drealincome[r,ir]], labels=["Wage" "Profit" "Total"], title="USA")
savefig("figtemp/dincomebytypeUSA1.pdf")
=#

#=
dX = 100 .* (log.(reshape(p.X[:,1], p.S, p.N)) .- log.(p.Xss))
