...
function plotelas(est, fit, fname)
    ...
    ls = [LineElement(color=col1, linewidth=lw1),
        LineElement(color=col2, linewidth=lw2, linestyle=:dash)]
...
function plotwelfare(df, dflr, H, d1, d2, n1, n2, tag)
    ...
    Legend(f1[2,1:2], ls, ["Total Effect", "No Distortion Term", "No Friction"],
...
function main()
    #elas = matread(string(elasdir/"elasparam.mat"))
    #plotelas(elas["estBaseline"], elas["fitBaseline"], "elasparam")
