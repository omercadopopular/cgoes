...
function plotbymodel(dfsufs, dests, tags, fsuf)
    df = DataFrame(readstat(modeldir/"welfare_$(dfsufs[1]).dta"))
    dflr = DataFrame(readstat(modeldir/"welfare_$(dfsufs[2]).dta"))
    df0 = DataFrame(readstat(modeldir/"welfare_$(dfsufs[3]).dta"))
    d1, d2 = dests
    n1, n2 = tags
    H = 12
    df1 = df[(df.iso3.==d1).&(df.h.<=H), :]
...
function main()
    dfsufs = ["Gtry3", "Glrtry03", "Gtry03"]
    plotbymodel(dfsufs, ("USA", "CN1"), ("US", "China"), "_USACHN")
    plotbymodel(dfsufs, ("MX1", "VNM"),  ("Mexico", "Vietnam"), "_MEXVNM")
end