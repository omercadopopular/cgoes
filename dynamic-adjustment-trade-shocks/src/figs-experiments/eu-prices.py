...
mdf = pd.melt(df[['dp','horz']].rename(columns=cols), id_vars=['horz'])
...
mdf2 = pd.melt(df[['dp', 'horz']].rename(columns=cols), id_vars=['horz'])
...
plt.savefig(os.path.join(path, 'figs/eu-prices.pdf'), bbox_inches='tight')
plt.show()
