...
class IndustryTariffs:
    ...
    def folder_walker(self, year, walk_folder='', year_check=True):
        base_path = os.join.path(self.folder, walk_folder)
        out_files = []
    ...
    def mfn_annual(self, year):
        ...
        for file_path in input_list:
            frame = frame.append(pd.read_csv(file_path))            
    ...
    def set_of_countries(self):
        ...
        for year in range(first_year, last_year):
            ...
            if not os.path.isfile(os.path.join(self.temp_folder, 'wits_mfn_' + str(year) + '.csv')):
                self.mfn_annual(year)
                                
            frame = os.path.join(self.temp_folder, 'wits_mfn_' + str(year) + '.csv')
            setc = setc.union(set(frame['Reporter_ISO_N']))
            
        return setc
            
    def process_bilateral_mfn_panel(self, reporter):
        first_year = self.first_year
        last_year = self.last_year
        setc = self.set_of_countries()
            ...
            for partner in setc:
                ...
                if partner == reporter:
                    ...
                    for column in columns:
                        frame.loc[partnerframe.index, column] = [0.0 for x in partnerframe[column]]
         
                partnerframe['Partner_ISO_N'] = partner
                yearframe = pd.concat([yearframe, partnerframe])
            
            outframe = pd.concat([outframe, yearframe])
