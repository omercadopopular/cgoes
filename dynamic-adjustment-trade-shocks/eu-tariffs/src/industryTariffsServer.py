...
class IndustryTariffs:
    def __init__(self, folder, first_year=1995, last_year=2010):
        ...
        self.temp_folder = os.path.join(self.folder, 'data/wits-out/temp')
    ...
    def mfn_annual(self, year):
        print(f'Organizing MFN Data for {year}.')
    ...
    def process_bilateral_full_panel(self, skip=True):
        first_year = self.first_year
        last_year = self.last_year
        ...
        for reporter in setc:
            print(f'Bilateral Panel, initiating reporter {reporter}')
            ...
            if not os.path.isfile(path):
                self.bilateral_mfn_panel(reporter)
            ...
            for partner in setc:
                print(f'Bilateral Panel, reporter {reporter}, partner {partner}')

                if partner == reporter:
                    
                    pframe = pd.concat([pframe, bframe])
            ...
            print(f'Bilateral Panel, finished reporter {reporter}')
...
# Usage
folder = '/u/main/tradeadj/euro-tariffs/'
print(folder)
tariffs = IndustryTariffs(folder)
print(tariffs)

tariffs.process_bilateral_full_panel()
