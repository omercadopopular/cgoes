...
class tariffFill():
    def __init__(self, folder, file, column_mapping = {}, first_year=1995, last_year=2018):
        ...
        self.rtadata_folder = os.path.join(self.folder, 'rta-data/')
    ...
    def fill_missing_mfn_tariffs(self):
        ...
        wto_data['MFN'] = wto_data.groupby(['Product', 'Partner'])['MFN'].ffill()
                ...
                for product, prod_data in period_data.groupby('Product'):
                    ...
                    if not phase_mask.all():
                        ...
                        if not np.isnan(last_phase_value):
                            filled_values.iloc[0] = last_phase_value
    ...
    def process_tariffs(self):
        ...
        output_data = output_data.sort_values(['Partner', 'Product', 'year'])
...
folder = r"C:\Users\andre\OneDrive\UCSD\Research\cgoes\dynamic-adjustment-trade-shocks\eu-tariffs\trottner"
file = 'full_ARE.csv'

tariff = tariffFill(folder, file, column_mapping=column_mapping)
