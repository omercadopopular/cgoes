...
cap log close
log using "./output/logs/log_r02b.smcl", append

////////////////////////////
///All data / MFN tariffs///
////////////////////////////

*Instrument.
local iv_opts iv_0_did
		...
		foreach lhs of local outcomes{
			...
			*Define matrix to store results.
			...
			forvalues h = 0 (1) `post'{
				...
				*Save results.
				xsvmat elasticity_`lhs', name(col) saving("./output/temp_files/`iv'_elasticity_`lhs'_l`l'.dta", replace)
...
//////////////////////////
///Top 5 major partners///
//////////////////////////

*Instrument.
local iv_opts iv_0_top5
		...
		foreach lhs of local outcomes{
			...
			*Define matrix to store results.
			...
			forvalues h = 0 (1) `post'{
				...
				*Save results.
				xsvmat elasticity_`lhs', name(col) saving("./output/temp_files/`iv'_elasticity_`lhs'_lags`l'.dta", replace)
				xsvmat elasticity_`lhs', name(col) list(,)
