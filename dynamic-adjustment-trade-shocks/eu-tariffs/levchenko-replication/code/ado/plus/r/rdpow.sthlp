...
{viewerjumpto "Saved results" "rdpow##saved_results"}{...}
{viewerjumpto "References" "rdpow##references"}{...}
{viewerjumpto "Authors" "rdpow##authors"}{...}


{title:Title}

{p 4 8}{cmd:rdpow} {hline 2} Power calculations for Regression Discontinuity designs using robust bias-corrected local polynomial inference.{p_end}


{marker syntax}{...}
{title:Syntax}

{p 4 8}{cmd:rdpow} {it:depvar} {it:runvar} {ifin} 
[{cmd:,} 
{cmd:c(}{it:#}{cmd:)} 
{cmd:tau(}{it:#}{cmd:)} 
{cmd:alpha(}{it:#}{cmd:)} 
...
{p 4 8}{cmd:rdpow} provides power calculations in Regression Discontinuity designs using conventional and robust bias-corrected local polynomial methods.
Companion command is: {help rdsampsi:rdsampsi} for sample size calculations.{p_end}
...
{p 8 8}{cmd:. rdpow demvoteshfor2 demmv, tau(5)}{p_end}

{p 4 8}Power calculation with covariates{p_end}
{p 8 8}{cmd:. rdpow demvoteshfor2 demmv, tau(5) covs(population dopen dmidterm)}{p_end}

{p 4 8}Power calculation with user-specified bandwidths{p_end}
{p 8 8}{cmd:. rdpow demvoteshfor2 demmv, tau(5) h(16 18) b(18 20)}{p_end}
