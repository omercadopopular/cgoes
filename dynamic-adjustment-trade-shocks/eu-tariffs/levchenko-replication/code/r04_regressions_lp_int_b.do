...
cap log close
log using "./output/logs/log_r04b.smcl", append
...
foreach l of local lags{
	...
	foreach iv of local iv_opts{
		...
		*Define matrix to store results.
		...
		local lp1 =`l'+1
		forvalues h = `pret'(-1)`lp1' {
	
			*Run regression.
			ivreghdfe L`h'.D0ln_tariff L(1/`l').D0ln_trade_val (D0ln_tariff L(1/`l').D0ln_tariff = `iv' L(1/`l').`iv'), absorb(`fe') first cluster(`clustvar')
	
			*Store results.
			mat pretrend_tariff[`h',1]= -1*`h'
			mat pretrend_tariff[`h',2]=_b[D0ln_tariff]
			mat pretrend_tariff[`h',3]=_se[D0ln_tariff]
			mat pretrend_tariff[`h',4]=_b[L1.D0ln_tariff]
			mat pretrend_tariff[`h',5]=_se[L1.D0ln_tariff]	
			mat temp=e(first)
			...
			*Save results.
			xsvmat pretrend_tariff, name(col) saving("./output/temp_files/`iv'_lp_tariffs_pre_l`l'.dta", replace)
		...
		*Define matrix to store results.
		...
		forvalues h = 1(1) `post' {
			...
			*Run regression.
			ivreghdfe D`h'ln_tariff L(1/`l').D0ln_trade_val (D0ln_tariff L(1/`l').D0ln_tariff = `iv' L(1/`l').`iv'), absorb(`fe') first cluster(`clustvar')
			
			*Store results.
			mat post_tariff[`h',1]= `h'
			mat post_tariff[`h',2]=_b[D0ln_tariff]
			mat post_tariff[`h',3]=_se[D0ln_tariff]
			mat post_tariff[`h',4]=_b[L1.D0ln_tariff]
			mat post_tariff[`h',5]=_se[L1.D0ln_tariff]	
			mat temp=e(first)
			...
			*Save results.
			xsvmat post_tariff, name(col) saving("./output/temp_files/`iv'_lp_tariffs_l`l'.dta", replace)
		...
		foreach lhs of local outcomes{
			...
			*Define matrix to store results.
			...
			forvalues h = `pret'(-1)`lp1' {
				...
				*Run regression.
				ivreghdfe L`h'.D0`lhs' L(1/`l').D0`lhs' (D0ln_tariff L(1/`l').D0ln_tariff = `iv' L(1/`l').`iv'), absorb(`fe') first cluster(`clustvar')

				*Store results.
				mat pretrend_`lhs'[`h',1]= -1*`h'
				mat pretrend_`lhs'[`h',2]=_b[D0ln_tariff]
				mat pretrend_`lhs'[`h',3]=_se[D0ln_tariff]
				mat pretrend_`lhs'[`h',4]=_b[L1.D0ln_tariff]
				mat pretrend_`lhs'[`h',5]=_se[L1.D0ln_tariff]	
				mat temp=e(first)
				...
				*Save results.
				xsvmat pretrend_`lhs', name(col) saving("./output/temp_files/`iv'_lp_`lhs'_pre_l`l'.dta", replace)
			...
			*Define matrix to store results.
			...
			forvalues h = 0 (1) `post'{
				...
				*Run regression.
				ivreghdfe D`h'`lhs' L(1/`l').D0`lhs' (D0ln_tariff L(1/`l').D0ln_tariff = `iv' L(1/`l').`iv'), absorb(`fe') first cluster(`clustvar')

				*Store results.
				mat post_`lhs'[`h'+1,1]= `h'
				mat post_`lhs'[`h'+1,2]=_b[D0ln_tariff]
				mat post_`lhs'[`h'+1,3]=_se[D0ln_tariff]
				mat post_`lhs'[`h'+1,4]=_b[L1.D0ln_tariff]
				mat post_`lhs'[`h'+1,5]=_se[L1.D0ln_tariff]	
				mat temp=e(first)
				...
				*Save results.
				xsvmat post_`lhs', name(col) saving("./output/temp_files/`iv'_lp_`lhs'_l`l'.dta", replace)
