...
collapse (first) ufCode ufName uf microregion_code_ibge (sum) employment, by( microregion_name isic3code3d year )
...
collapse (sum) gGDPbasecont, by(year uf isic3code3d)
...
merge 1:m year uf isic3code3d using "data\temp\laborShrs.dta"
drop if _merge == 1
...
collapse (sum) giv* (first) LaborShrbase, by(year ufCode uf ufName microregion_name microregion_code)
...
save "data\temp\microregion-trade-panel-inst-base.dta", replace
