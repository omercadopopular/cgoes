...
bysort microregion_code_ibge: egen LaborShr2000 = mean(t_LaborShr2000)
gen t_wr2000 = wr if year == 2000
bysort microregion_code_ibge: egen wr2000 = mean(t_wr2000)
drop t_*
	...
	foreach var of global lhs {
		...
		*Define matrix to store results.
		...
		forvalues h = 0 (1) $leads {
			...
			*Save results.
			xsvmat results_`var', name(col) saving("results/temp/`iv'_results_`var'_alt.dta", replace)
...
qui reghdfe giv_comtrade l(1/4).giv_comtrade, absorb(uf_code)
local IRF0 = 1
local IRF1 = `IRF0'
