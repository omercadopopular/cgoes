...
egen groupid = group(uf isic3code3d iso3code)
xtset groupid year
...
collapse (sum) gGDPcont g2GDPcont dGDPcont, by(year uf isic3code3d)
...
merge 1:m year uf isic3code3d using `laborShrs'
drop if _merge == 1
...
collapse (sum) employment female less_than_primary primary secondary college_or_higher wage_mass wage_mass_sexo_1 wage_mass_less_than_pri wage_mass_secondary wage_mass_college giv g2iv div LaborShr (mean) avg_wage_q1 avg_wage_q2 avg_wage_q3 avg_wage_q4, by(year ufCode uf ufName microregion_name microregion_code)
...
save "data\temp\microregion-trade-panel-inst.dta", replace
