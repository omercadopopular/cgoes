...
Path = r'C:\Users\cbezerradegoes\OneDrive - WBG\Brazil'
...
FrameSum.write_csv(os.path.join(outPath, 'EXP_1997-2023.csv'), separator=',')


## define tradeables and nontradeables by state
tradeableFrame = FrameSum.group_by(['CO_ANO', 'SG_UF_NCM', 'CO_ISIC_CLASSE'] ).agg( pl.col('VL_FOB').sum() )
...
tradeableFrame = tradeableFrame.with_columns(
    ...
    )[['CO_ANO', 'SG_UF_NCM', 'CO_ISIC_CLASSE', 'q1','q2','q3','q4']]

tradeableFrame.write_csv(os.path.join(outPath, 'EXP_quartiles_4digit.csv'), separator=',')
