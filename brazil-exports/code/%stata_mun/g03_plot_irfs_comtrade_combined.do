...
foreach var of local group {
	di "`var'"
		...
		twoway rcap high_male low_male horizon, color(red%50) || ///
			...
			graph export "figs\\m`iv'_male_female.pdf", as(pdf) replace
			graph export "figs\\m`iv'_male_female.wmf", as(wmf) replace
...
foreach var of local group {
	di "`var'"
		...
		twoway rcap high_less_than_college low_less_than_college horizon, color(red%50) || ///
			...
			graph export "figs\\m`iv'_college.pdf", as(pdf) replace
			graph export "figs\\m`iv'_college.wmf", as(wmf) replace
...
foreach var of local group {
	di "`var'"
		...
		twoway rcap high_w_male low_w_male horizon, color(red%50) || ///
			...
			graph export "figs\\m`iv'_male_female_w.pdf", as(pdf) replace
			graph export "figs\\`miv'_male_female_w.wmf", as(wmf) replace
