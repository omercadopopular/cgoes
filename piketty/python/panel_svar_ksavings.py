...
moments_mat, median_mat, average_mat, pctiles = panel_var.collect_moments(irfs, pcttiles_method='linear')
fig_moments = panel_var.plot_moments(variables, moments_mat, median_mat, average_mat, pctiles, figsize=(14,14), titles_dict=labels)
fig_moments.savefig(os.path.join(figs_path, 'moments_ksavings.pdf'))

# Plot decomposed median IRFs:
median_common, median_id = panel_var.decompose_irfs(variables, lags=1, pcttiles_method='linear')
fig_decomp = panel_var.plot_decomposed_irfs(median_common, median_id, figsize=(14,14), titles_dict=labels)
fig_decomp.savefig(os.path.join(figs_path, 'decomp_ksavings.pdf'))

# Calculate and Plot Standard Erros of Medians
median_mat_se, average_mat_se = panel_var.calculate_se(variables, lags=1)
fig_se = panel_var.plot_se(median_mat, median_mat_se, variables, figsize=(14,14), titles_dict=labels)
fig_se.savefig(os.path.join(figs_path, 'median_se_ksavings.pdf'))
