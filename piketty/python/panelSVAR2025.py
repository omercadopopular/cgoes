...
class PanelVAR:
    ...
    def estimate_structural_var(self, variables: list, lags: int, data=None):
        ...
        Returns:
        - dict: A dictionary where keys are countries and values are VAR model results.
        """
        if data is None:
            data = self.data
    ...
    def collect_moments(self, mat_col: np.array, pctiles = [25,75], pcttiles_method = 'closest_observation'):
        ...
        Returns:
        - dict: A dictionary where keys are countries and values are IRF matrices (horizon x variables).
        """
    ...
    def estimate_structural_var_on_averages(self, variables: list, lags: int):
        ...
        averaged_data = self.data.groupby(self.time_col)[variables].mean().reset_index()
    ...
    def decompose_irfs(self, variables, lags, pcttiles_method='closest_observation'):
        ...
        for idx in factors.keys():
            for h in range(self.horizon):
                ...
                mat_id[:,:,h,idxx] = mat_col[:,:,h,idxx] - mat_common[:,:,h,idxx]
            idxx += 1
    ...
    def plot_decomposed_irfs(self, median_common, median_id, titles_dict={}):
        ...
        for i in range(n):
            for j in range(n):
                ax[i,j].bar(r, median_common[i,j,:], color='black', label='Common')
                ax[i,j].bar(r, median_id[i,j,:], bottom=median_common[i,j,:], color='grey', label='Idiosyncratic')
                ax[i,j].axhline(y=0, color='black')
                ax[i,j].set_title(f"Shock: {titles[j]}. Response: {titles[i]}")
                ax[i,j].grid(True)                    
    ...
    def calculate_se(self, variables, lags, draws=1000):
        ...
        for draw in range(draws):
            ...
            for idx, group_data in base_data.groupby(self.idx_col):
                ...
                P = var_results[idx]['P']
                u = np.matmul(np.linalg.inv(P), eps)
    ...
    def plot_se(self, central_mat, central_mat_se, variables, t_val=1.96, titles_dict = {}):
        ...
        for i in range(n):
            for j in range(n):
                ax[i,j].plot(central_mat[i,j,:], '*-', color='navy')
                ax[i,j].axhline(y=0, color='black')
                ax[i,j].set_title(f"Shock: {titles[j]}. Response: {titles[i]}")
                ax[i,j].grid(True)
