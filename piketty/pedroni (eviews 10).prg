...
''''' DIRECTORY SETTINGS
	...
	%dir = "C:\Users\Carlos\OneDrive\Documentos\Piketty"									'Working directory
	%wfname = "piketty"													'Workfile name
	%filename = "fdatabasetax.csv"									'CSV source file
	%save = "finallevelstax"
''''' PANEL SETTINGS
...
''''' DATA ADJUSTMENTS SETTINGS
	...
	%group = "rg2 savings top1 growth long"							'Raw data to be transformed
	...
	!diftransformation = 1													'Do log difference transformation? YES = 1 (resulting variable will be d{variable_name})
	!delete = 0																	'Delete all the calculation objects? YES = 1 
	
'''' VAR SETTINGS

	!endogenous = 2 														'Number of endogenous variables
	%cholesky	= "frg2 ftop1"												'List of endogenous VARIABLES (if needed, use log transformed data)
	!maxlags = 3																'Max number of lags in VAR
...
''' Create group of endogenous variables
...
''' Create strings of residuals
	...
	string cholesky_0 = ""
		...
		for !k=1 to !endogenous
			string s_resid = s_resid + "r_" + endogenous.@seriesname({!k}) + " "
			string s_tresid = s_tresid + "tr_" + endogenous.@seriesname({!k}) + " "
			string s_bresid = s_bresid + "br_" + endogenous.@seriesname({!k}) + " "
			string cholesky_0 = cholesky_0 + endogenous.@seriesname({!k}) + "_0" + " "
		next

	string s_resid1 = ""

		for !k=1 to 1
			string s_resid1 = "r_" + endogenous.@seriesname({!k}) + " <> na"
		next
...
''RUN COMMON SHOCKS REGRESSION (TIME EFFECTS)
	'' DETERMINE LAG LENGTH
		...
		for !kk = 1 to !endogenous
			...
			next
		next
		common.makeresid {s_tresid}
		
		for !k = 1 to !endogenous
			string tvar = "r_" + endogenous.@seriesname({!k}) 
			string tvar2 = "tr_" + endogenous.@seriesname({!k})
			series {tvar} = {tvar2}
		next

		group resid_g {s_resid}
...
for !ind = 1 to maxcross 
		...
		individual_{!ind}.makeresids {s_tresid}
...
for !rep=1 to !reps
	...
	''RUN COMMON SHOCKS REGRESSION (TIME EFFECTS)
		...
		'' DETERMINE LAG LENGTH
			...
			b{!rep}_common.makeresid {s_tresid}
			group b{!rep}_resid_g {s_tresid}
	...
	for !ind = 1 to maxcross 
			...
			smpl @all if crossid = !ind and {s_resid1}
			...
			b{!rep}_individual_{!ind}.makeresid 	{s_tresid}
		
			for !k = 1 to !endogenous
				string tvar = "br_" + endogenous.@seriesname({!k}) 
				string tvar2 = "tr_" + endogenous.@seriesname({!k})
				series {tvar} = {tvar2}
			next

			group b{!rep}_resid_g{!ind} {s_bresid}
