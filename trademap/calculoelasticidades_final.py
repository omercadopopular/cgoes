...
l = plt.plot(bins, y, 'r--', linewidth=2, label="Normal Dist. $(\mu, \sigma=1)$")

# Linha descrevendo a elasticidade média
plt.axvline(np.mean(elasticities_trimmed), color='black', label="Mean") 
...
ax.scatter(elasticities, tvalues, color='grey', alpha=0.25, label="Elasticities") 

# Linha descrevendo a elasticidade média
ax.axvline(np.mean(elasticities), color='black', label="Mean") 
...
plt.xlabel('Elasticity') 
plt.ylabel('t-value')  
plt.title('Elasticities and t-values')
plt.show()
...
# Linha descrevendo a média
plt.axvline(np.mean(modelresult_micro['pop_change_mun_final'].dropna()), color='black', label="Mean") 
         
# Formatação
ax.legend(loc='upper left')
plt.xlabel('Expected pct change') 
plt.ylabel('Probability of Each Value')  
plt.title('Histogram of micro-region changes')
plt.show()
